...
\subsection{Derivation of Eq. (B3)}
...
-\log
\int
\frac{ 2 \cosh(\Delta \beta E) + 2 \exp(\Delta f) }
{
  1 +  2 \cosh(\Delta \beta E) \, \exp(\Delta f)
  + \exp(2 \Delta f)
}
\rho_1(E) \, dE
\notag \\
...
-\log
\int
\frac{ \cosh(\Delta \beta E) + \exp(\Delta f) }
{
  \cosh(\Delta \beta E) + \cosh(\Delta f)
}
\rho_1(E) \, \exp(-\Delta f) \, dE
\notag \\
&=
\Delta f
-
...
\label{eq:delf_WHAM}
\end{align}
where we have used Eq. \eqref{eq:rho_symm}.



\subsection{Derivation of Eq. (B4)}


From Eq. \eqref{eq:delf_WHAM},
\begin{align}
\mathrm{r.h.s.}
&=
\Delta f
...
%
where
we have used Eq. \eqref{eq:rho_normalize}.
