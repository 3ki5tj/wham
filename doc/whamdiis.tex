...
\begin{figure}
  \begin{tikzpicture}
    ...
    \node (Sf2) at ({\Sox - 1.0*\Ssx}, {\Soy})
      ...
      edge[fRarrow2] (Sf2R2);
    %
    \node (SfhatRhat) at ({\Sox - \Ssx/3}, {\Soy + 0.206*\Ssy})
      [emptydot,
        label={[label distance=0.1mm]
               90:{$\vct {\hat R}$}}
      ] {};
...
\begin{figure}[h]
  ...
  \caption{
    ...
    or $10^{-7}$ for MBAR.
    %
    For MBAR, only about 1\% of the trajectory frames
    were randomly selected to accelerate the calculation.
    %
...
%
Among the approximate formulae
...
Eq. \eqref{eq:df_eavb},
but not Eq. \eqref{eq:df_eavc},
appeared to made an improvement.
...
\definecolor{darkgray}{rgb}{0.2,0.2,0.3}
{\color{darkgray}
