...
and the closely-related multiple Bennett acceptance ratio (MBAR) method
...
can suffer from
slow convergence in the later stages\cite{
...
the implementation of WHAM and MBAR using
...
the $K$ canonical ensembles\cite{chodera2007},
...
%
In this sense,
MBAR is not only the zero-bin-width limit of WHAM\cite{shirts2008, tan2012},
...
%
where
$n_k(W) \equiv \sum_{\vx}^{(k)} \delta(\W(\vx) - W)$
is understood to be
...
with $\sum_{k = 1}^K n_k(E') \, \Delta E = 0$,
...
$\vct{\hat R} = \sum_{i = 1}^M c_i \, \vct R_i$,
...
%
\begin{equation}
  \sum_{j = 1}^M \left( \vct R_i \cdot \vct R_j \right) \, c_j = \lambda,
  \label{eq:cj_DIIS}
\end{equation}
%
for all $i$,
...
\begin{center}
  ...
  \caption{\label{fig:scheme}
    ...
    (b) and (c)
    %
    Given a basis of $M$ (two here) trial vectors,
    DIIS seeks the combination
    $\vct {\hat R} = \sum_{i = 1}^M c_i \, \vct R_i$
    that minimises the magnitude
    $\| \vct{\hat R} \|$
    under the constraint
    $\sum_{i = 1}^M c_i = 1$ [panel (c)].
    %
...
\begin{center}
  ...
  \caption{
    ...
    $12.5$, $1.87\times 10^3$, $14.3$, and $260$ seconds,
    respectively.
    %
    ...
    for MBAR.
    To mimic the correlation in the trajectories,
...
denotes an average over trajectory $i + 1$.
%
...
%
The first is\cite{park2007, zhu2012}
%
\begin{equation}
\Delta f_i
\approx
...
which can be difficult for ST-WHAM\cite{kim2011}.
%
...
with temperatures $T = 1.2, 1.3, \dots, 1.7$,
and pressures $p = 0.1, 0.15, 0.2$.
%
...
in aqueous solution.
%
This is a well-known test system\cite{duan1998}.
%
...
%
although MBAR was slower than WHAM
...
which is the zero-bin-width limit of WHAM\cite{shirts2008, tan2012}.
%
...
%
Generally,
WHAM was insensitive to the bin size,
whereas ST-WHAM was slightly affected
by a small bin size for the smaller samples.
...
Probabilistic derivations of Eq. \eqref{eq:f_MBAR}}



Here we give some derivations of Eq. \eqref{eq:f_MBAR}.
First, we show that
Eq. \eqref{eq:f_MBAR}
...
\begin{equation}
  ...
  \label{eq:likelihoodNk}
\end{equation}
%
with respect to variations of $N_i$
under the constraint $\sum_{i = 1}^K N_i = N_\mathrm{tot}$\footnote{
  \hl{
  Note that varying $N_i$ under fixed $Z_i$
  is equivalent to inversely varying $Z_i$ under fixed $N_i$,
  i.e., $\delta N_k \rightarrow -N_k \, \delta \log Z_k$,
  we can treat Eq. \eqref{eq:likelihoodNk}
  as a functional of $Z_i$, under the constraint that
  $\sum_k N_k \, \log Z_k$ is a constant.
  Further, we can show that the Lagrange multiplier is $1.0$,
  and obtain a functional of $Z_i$\cite{zhu2012, tan2012}:
  \[
  \sum_{\vx}
  \log \sum_{ k = 1 }^K
  \frac{ N_k }{ N_\mathrm{tot} } \, w_k(\vx)
  + \sum_{k = 1}^K N_k \, \log Z_k.
  \]
  }
}.
%
...
can be set to any values that help calculation,
...
and
$D(E; \vct f^*) = \exp\left( \frac{1}{2} \Delta \beta E \right) J(E)$,
with
\begin{align*}
  J(E)
  &\equiv
...
\end{align*}
where
$\theta = 1 + \frac 1 2 \exp\left[-u \Delta \beta^2 / (4 \, a) \right]$.
%
