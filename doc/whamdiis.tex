...
For comparison,
we also computed \repl{the dimensionless free energies}{$f_i$} from two approximate formulae.
%
...
we have\footnote{
%
For $A_{12}$, the key steps are
\begin{align*}
D(E; \vct f^*)
&\propto
2 \cosh\left(
  \tfrac{1}{2} \Delta \beta \Delta E
\right)
+
\exp\left(
  -\tfrac{3}{2} \Delta \beta \Delta E
...
\left.
  \exp\left(
    \tfrac{\Delta \beta^2 \Delta E^2} { 8 \, \theta }
  \right)
\middle/
  \left[
    1
    +
    \tfrac{ (3 - \theta) \Delta \beta^4 \Delta E^4 }
    { 24 \cdot 2^4 \theta^2 }
  \right]
\right.,
\end{align*}
where
$\Delta E = E + \frac{\beta_1 + \beta_2}{2 \, a}$
and
$\theta = 1 + \frac 1 2 \exp\left(-\frac 1 4 u \Delta \beta^2 \, \Delta E^2\right)$.
%
The parameter $u$ should be $5/2$
to match the small $\Delta \beta$ behavior,
although the value $u \approx 2.11$
gives better results in overall accuracy.
}
%
\begin{align}
1 - \lambda_1
