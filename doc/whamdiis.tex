...
\end{equation}
%
The $K = 1$ case of Eq. \eqref{eq:gE_WHAM}
follows from definition,
and the $K > 1$ case can be considered
...
%
%
Using Eq. \eqref{eq:n_delta} in Eq. \eqref{eq:f_WHAM} yields\cite{
...
%
Note that both Eqs. \eqref{eq:f_WHAM} and \eqref{eq:f_MBAR}
are invariant under $f_i \rightarrow f_i + c$
for all $i$ and an arbitrary $c$.
%
Thus, $f_i$ are determined up to a constant shift.
...
%
The approximation should not greatly affect
the general performance of the methods.
...
\begin{figure}[h]
  ...
  \caption{
    ...
    from Refs. \cite{ferdinand1969} and \cite{beale1996},
    respectively.
  }
\end{figure}
...
%
However, the method is still iterative,
and thus,
...
%
UIM also offers a non-iterative,
albeit more approximate, alternative.
...
\end{equation}
%
For on-the-fly estimates
in simulated tempering,
Eqs. \eqref{eq:df_eav}
and \eqref{eq:df_eavx}
are more suitable
than the WHAM-like methods.
%
Below, we show that
Eq. \eqref{eq:df_eavx}
is preferred
for simulations of large systems
in a finite temperature range.



The last term on the right-hand side of Eq. \eqref{eq:df_eavx}
shows that the accuracy
of Eq. \eqref{eq:df_eav}
...
the accumulative error of the
dimensionless free energy
is $O(N \, \Delta \beta^2)$,
which is negligible only
for a sufficiently small $\Delta \beta$.
%
However,
in a common tempering simulation,
one sets up the temperatures
...
is a multiple of the width of
the energy distribution
$\sqrt{ \langle \Delta E^2 \rangle }$,
which is $O(\sqrt{N})$.
%
So $\Delta \beta$,
...
of dimensionless free energy, $f_i$,
is $O(1)$.
%
If the $f_i$ from Eq. \eqref{eq:df_eav}
are used for the weights
in simulated tempering\cite{park2007},
there can a non-negligible deviation of
the resulting temperature distribution
from the designed one.



In Fig. \ref{fig:is2approx}
the accuracy of
Eqs. \eqref{eq:df_eav},
\eqref{eq:df_eavx},
was compared with the result of
UIM\cite{kastner2006}
(which is also approximate).
%
We used the exact results\cite{
ferdinand1969}
for
$\langle E \rangle$
and
...
%
Equation \eqref{eq:df_eav}
produced an $O(1)$ error
in the accumulated free energy
with $\Delta T = 0.1$,
while
the error of Eq. \eqref{eq:df_eavx}
was negligible.
...
\begin{figure}[h]
  ...
  \caption{
    ...
    and UIM for $32\times 32$ Ising model.
    %
