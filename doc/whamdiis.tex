...
There are two non-iterative alternatives\cite{
%shen1991, woolf1994, *crouzy1994,
...
%
The method can be regarded as a refinement of
the more approximate umbrella integration method (UIM)\cite{
kastner2005, *kastner2009},
and both methods are non-iterative,
for they avoid the free energies in constructing
the density of states.
%
ST-WHAM, however,
can be sensitive to the histogram bin size,
%gives a slightly different estimate of the free energy from the original WHAM,
and the extension to multidimensional ensembles,
e.g., the isothermal-isobaric ensemble,
can be numerically challenging\cite{kim2011}.
%
UIM is more stable, but less rigorous.
...
Here we follow the probabilistic approach\cite{
...
\end{equation*}
%
If we assume a uniform prior distribution,
$p[g(\vy)]$, of the value of $g(\vy)$,
at each phase-space point $\vy$,
then Bayes' theorem\cite{leonard} states that
...
in Eq. \eqref{eq:Zg},
and finally setting the $g$ in $Z_k[g]$ to 1.0
