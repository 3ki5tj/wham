...
%
\note{
Should we merge the above two sentences?
In this work, we use a well-known linear algebra algorithm,
the method of direct inversion in the iterative subspace,
for more rapid convergence to the solution of the WHAM equation.
}
%
...
%
\note{
The term ``Hessian'' from optimisation problems
...
labelled by\del{ the inverse temperature,}
$\beta = 1/(k_B T)$,
as
$\beta_1, \ldots, \beta_K$.
%
...
\repl{$g(E)$}{$g(E) = \int \delta(\E(\vx) - E) \, d\vx$,
...
\repl{$(E - h/2, E + h/2)$
divided by $h$}{$(E - \Delta E/2, E + \Delta E/2)$ divided by $\Delta E$}
...
\add{the single histogram estimate}
%
\begin{equation}
g(E)
=
...
should \add{roughly} match the exact one, $g(E) \, w_k(E)$,
...
{
$\mathrm{var}(n_k \, \Delta E) \approx \langle n_k \rangle \, \Delta E$,
}
%
\note{
...
$\int n_k(E) \, dE = N_k$.
%
This constraint has to be ignored in order
to assume the Poisson distribution of each $n_k(E) \, \Delta E$,
i.e., each bin is an independent variable.
%
We probably can get away from such subtleties here,
for this supposedly quick-and-dirty derivation.
}
%
we have \repl{$\mathrm{var}(n_k/d_k) \approx n_k / d_k^2 \propto 1/d_k$}{
...
%
Since for a fixed $E$,
$d_k(E)$ is proportional to \repl{$n_k(E)$}{$\langle n_k(E) \rangle$},
...
\repl{the MBAR result}{the histogram-free, or the MBAR, form}\cite{
kumar1992, souaille2001, shirts2008}:
%
\begin{equation}
f_i
...
%
for all $i$,
with $\lambda$ being \repl{the}{an unknown} Lagrange multiplier
\add{that is to be determined from along with $c_i$}.
%
...
(or, in our case, $K$
because of the arbitrary shift constant \add{of $f_i$})
...
\begin{center}
  ...
  \caption{
    ...
    \note{In the Ising model, the label $NVT$ is removed,
      ...
      %
      There are people who insist $V$ as the magnetisation
    ...
    by a factor $\tau$
    for better \repl{comparison}{alignment with the numbers of iterations}.
    %
...
the \repl{general}{convergence} behaviour of the methods.
...
%
\repl{This formula can be improved by an}{The second is an improvement by the}
...
\begin{center}
  ...
  \caption{
    ...
    \add{(plotted with an energy spacing of 200)}.
    %
    Here,
    ...
    %
    \add{Lines are a guide for the eyes.}
  }
\end{center}
\end{figure}
...
reduced the run time\repl{significantly.
This demonstration also shows that}{, although}
...
%
\del{
With a denser temperature set-up, however,
...
\repl{$g(\vy) \, q_k(\vy) / Z_k[g]$}{$w_k(\vy) \equiv g(\vy) \, q_k(\vy) / Z_k[g]$}, with
...
\repl{$\log p\left( g | \{ \vx \} \right)$}{$\log p\left( \{ \vx \} | g \right)$}
...
and
$\delta Z_k[g] / \delta g(\vy) = q_k(\vy)$.\footnote{\add{
...
as a Gaussian.
%
With a proper choice of the \repl{shift constant of
$\log g(E)$}{multiplicative constant of $g(E)$}, we have,
%
\begin{equation}
g(E)
=
...
can be set to any \repl{value
that facilitates}{values that help} calculations,
...
By further assuming \add{a }Gaussian density of states,
...
This model shows
a rapid decrease in
the \add{rate of} convergence of direct WHAM
