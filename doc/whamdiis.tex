...
\hladd{and the closely-related multiple Bennett acceptance ratio (MBAR) method}
...
the implementation of WHAM \hladd{and MBAR} using
...
the $K$ canonical ensembles\hladd{\cite{chodera2007}},
...
{
  \sum_{k = 1}^K N_k \, q_k(\vx) \exp \hladd{(}f_k\hladd{)}
}.
\label{eq:f_MBAR}
\end{equation}
%
...
\begin{center}
  ...
  \caption{
    ...
    $12.54$, $1866$, \hladd{$14.30$, and $259.9$} seconds,
    respectively.
    %
    ...
    for MBAR.
    \hladd{To mimic the correlation in the trajectories,
      we allow each data point in the bootstrap sample
      to be either a repetition of the previous one (if any)
      ...
      or drawn randomly from the trajectory.}
    %
...
(\hladd{after} integration by parts,
...
(PDB ID: 1VII),
\hladd{in aqueous solution},
which is a well-known test system\cite{duan1998}.
%
...
%
\hladd{Molecular dynamics (MD)} simulations were performed
using \hladd{GROMACS 5.0}\cite{
...
%
\hladd{although it was slower than WHAM
...
\hladd{which is the zero-bin-width limit of WHAM.
%
...
Probabilistic \hladd{derivations} of Eq. \eqref{eq:f_MBAR}}



Here \hladd{we give some derivations of Eq. \eqref{eq:f_MBAR}.
First,} we show that
Eq. \eqref{eq:f_MBAR}
...
(no matter the original state label $j$)
...
in the expectation-maximisation algorithm\cite{dempster1977,
...
%
\hladd{
Besides, sampling error, which we have ignored
in the above calculation in using EDA,
may decrease the rate of convergence further.
}
