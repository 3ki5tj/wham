...
and the villin headpiece \hladd{(}a small protein\hladd{)} in aqueous solution
(see Secs.
...
\begin{center}
  ...
  \caption{
    ...
    $12.54$, $1866$, \hladd{$14.30$, and $259.8$} seconds,
    respectively.
    %
    ...
    for MBAR.
    \hladd{To mimic the successive correlation in the trajectory,
      we choose each data point in the bootstrap sample
      to be either a repetition of the previous one (if any)
      with probability $\exp(-\Delta t_\mathrm{MD}/t_\mathrm{act})$,
      where $t_\mathrm{MD}$ and $t_\mathrm{act}$
...
Here \hladd{we give several derivations of Eq. \eqref{eq:f_MBAR}.
First,} we show that
Eq. \eqref{eq:f_MBAR}
...
\hladd{
Alternatively,
we can show Eq. \eqref{eq:f_MBAR} directly.
%
We no longer need the configuration-space field
and thus set $g(\vct x) = 1$ below.
%
Instead,
...
and the condition probability of observing
configuration $\vx$ in state $i$ is
%
\begin{equation}
...
The above argument is akin to the one used
the expectation-maximisation algorithm\cite{dempster1977, neal1998,
...
for given $\vct \nu = (\nu_1, \dots, \nu_K)$
regardless of the states $\{ k(\vx) \}$,
we sum over all possible combinations
of $\{ k(\vx) \}$:
%
\begin{align*}
  p\left( \{\vx \} | \vct \nu \right)
...
%
The probability gives the likelihood of $\vct \nu$
given trajectory $\{ \vx \}$.
%
We now maximise
$\log p\left( \{ \vx \} | \vct \nu \right)$
...
\begin{align}
  ...
  { \sum_{k = 1}^K \nu_k \, w_k(\vx) }
  =\lambda,
  \label{eq:posterior_em}
\end{align}
%
where $\lambda$ is the Lagrange multiplier of the constraint.
%
By mutiplying both sides of Eq. \eqref{eq:posterior_em}
by $\nu_i$ and summing over $i$,
we get $\lambda = \sum_{\vx} 1 = N_\mathrm{tot}$.
%
We then recover Eq. \eqref{eq:f_MBAR}
upon setting $\nu_k = N_k / N_\mathrm{tot}$
and using Eq. \eqref{eq:wi_qi}.
%
}
%
%
...
in using EDA in the above calculation,
