...
%
On the other hand,
the result from ST-WHAM
was slightly different
(possibly because of
the accumulative difference
in the density of states
from integrating the statistical temperature).
...
\begin{figure}[h]
  ...
  \caption{
    \label{fig:is2ref}
    (a) The dimensionless free energy and
    (b) the logarithm of the density of states
    for the $64\times64$ two-dimensional Ising model.
    %
    Here, $\varepsilon(a) \equiv a - a^\mathrm{(ref)}$.
    %
    For ST-WHAM,
    the derivative $n'_j(E)$ is estimated from
    $[\log n_j(E + \Delta E) - \log n_j(E - \Delta E)]/(2 \Delta E)$,
    with $\Delta E = 4$.
  }
\end{figure}
