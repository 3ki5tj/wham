...
as a weighted average\cite{souaille2001}.
...
by more accurate alternatives, such as ST-WHAM.
...
on the use of Eq. \eqref{eq:df_eav} for $f_i$,
especially for the on-the-fly weight estimates
...
in an $O(1)$ temperature range.
%
The correction of Eq. \eqref{eq:df_eavx}
to Eq. \eqref{eq:df_eav}
...
is $O(N \, \Delta \beta^2)$.
%
In a tempering simulation,
one often sets up the temperatures
...
is a roughly multiple of the width of
...
of $f_i$ is $O(1)$ [cf. Fig. \ref{fig:is2approx}].
%
Thus, if the $f_i$ from Eq. \eqref{eq:df_eav}
are used for the weights
in simulated tempering\cite{park2007},
there can be a non-negligible deviation of
