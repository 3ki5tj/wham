...
\note{WHAM is supposed to be ``unbiased'';
...
But the changed version is still not quite accurate,
because
distributions do not have to overlap for WHAM to work,
as discussed in Appendix \ref{sec:convwham}.
}
%
...
\repl{continuous functions (bins of zero width)\cite{
...
that manages to cast it as an extension of BAR in the case of more than one temperature.
In this sense, only Shirts's paper should be cited here.
The citations of the other two papers are added at the end of the paragraph.
}
%
This form,
...
\add{readily} handle such a case
for the Lennard-Jones (LJ) fluid\del{, readily}.
...
Dr. Y. Zhao, Dr. M. W. Deem, Dr. M. R. Shirts,
...
Here, we use several simple models
to study the convergence of direct WHAM and MBAR.



\subsection{Linearized WHAM/MBAR equation}



Consider $K$ distributions
that differ by the temperatures,
$\beta_i$, ($i = 1, \dots, K$),
normalized as
%
%\begin{equation}
...
Thus,
$\vct A$
can be regarded as a transition matrix.
%
The largest eigenvalue is $1.0$,
and the left eigenvector is $(1, \dots, 1)$,
which corresponds to a uniform shift of all
$\delta f_i$:
%
$
...
$\lambda_1$,
determines the rate of approaching the true solution,
as the slowest mode error reduction
is given by $\lambda_1^n$,
...
\end{equation}
%
with $g(E)$ being the density of states, and
%
\begin{align}
\sum_{k = 1}^K
\rho_k(E)
...
\end{align}
%
Using this in Eq. \eqref{eq:Aij}, we get
%
\begin{align}
A_{ij}
=
...
Further, we can approximate, $g(E)$,
as a Gaussian.
%
With a proper choice of the origin of
energy, $E$, and entropy, $\log g(E)$,
we can write,
%
\begin{equation}
g(E)
...
Assuming EDA, Eq. \eqref{eq:Aij_EDA},
we have
%
\begin{align}
A_{12}
...
\begin{figure}
  ...
  %
  %
  \caption{\label{fig:A12}
    Matrix element $A_{12}$
    as a measure of overlap of distributions 1 and 2.
  }
\end{figure}



Assuming the Gaussian model,
we have, from
...
\sqrt \frac{ a } { 8 \, \pi }
\int
\frac{ \exp\left[ -\frac{1}{2} a \, E^2 - \frac{ \Delta \beta^2 }{ 8 \, a } \right] }
{ \cosh( \Delta \beta \, E / 2 ) }
\, dE
\\
&\approx
...
{ 2 \sqrt{ 1 + z^2 } }
\left[
1 + \frac{z^4}{4\,(1+z^2)^2 }
\right]
\xrightarrow{z \gg 1}
\frac{5}{8} \frac{ e^{-z^2/2} }
{ z },
\end{align*}
%
where
$z = \Delta \beta / (2 \, \sqrt a)$,
and
we have used $1/\cosh x \approx \exp(-x^2/2)(1 + x^4/12)$.



The second largest eigenvalue is then given by
%
\begin{align}
\lambda_1 = 1 - 2 \, A_{12}
=
1 - \frac{5}{4} \frac{ \exp(-\sigma_\beta^2 \, \sigma_E^2/2) } { \sigma_\beta \sigma_E }.
\label{eq:lambda1_twoT}
\end{align}
%
where,
$\sigma_\beta = \Delta \beta/2$
and
$\sigma_E = 1/\sqrt{a}$.
%
This suggests that
with a larger temperature separation,
$\lambda_1$
quickly approaches 1.0,
leading to slow convergence
of direct WHAM and MBAR.




\subsection{Multiple-temperature case}
...
Below, we assume a Gaussian $\beta$ distribution:
%
\begin{align}
w(\beta)
=
...
%
By further assuming EDA, Eq. \eqref{eq:Aij_EDA},
and the Gaussian approximation,
Eq. \eqref{eq:f_Gaussian},
we get
%
\begin{align*}
D(E; f_1^*, \dots, f_K^*)
...
%
%
The $A_{ij}$ from Eq. \eqref{eq:Aij_EDA} becomes,
%
%
%
\begin{align*}
...
%
\begin{equation}
K \int c_l(\beta') \, w(\beta') \, A(\beta, \beta') \, d\beta'
= \lambda_l \, c_l(\beta),
\notag
\end{equation}
%
...
%
The second largest eigenvector can be written as
%
\begin{equation}
\lambda_1
=
...
\end{equation}
%
It also approaches $1.0$
with increasing temperature range, $\sigma_\beta$,
leading to slow convergence.
%
However,
the rate of increase
is smaller compared to that
of the two-temperature case, Eq. \eqref{eq:lambda1_twoT},
