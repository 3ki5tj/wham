...
\begin{abstract}
The convergence of the weighted histogram analysis method for free energy calculations
...
has made WHAM a popular trajectory analysis tool
...
%
Below,
we shall derive it from the most general
histogram-free form\cite{
...
%
Suppose we have run $K$ simulations
under different conditions.
%
In the $k$th ($k = 1, \dots, K$) simulation,
the configuration space distribution
is $q_k(\vy) / Z_k$,
with
$q_k(\vy)$
being a generalization of the Boltzmann factor,
and
$Z_k \equiv \int q_k(\vy) \, d\vy$
being the partition function.
%
We wish to find the dimensionless free energy
$f_k \equiv -\log Z_k$.



We shall assume that the system is subject to
...
\end{equation}
%
In the end, we shall switch off the field
by setting $g(\vy) \equiv 1$.
%
For now, however,
we shall seek the most probable $g(\vy)$
...
a set of independent trajectory frames,
$\{ \vx \}$,
is given by
%
\begin{equation*}
...
\end{equation*}
%
where the inner product runs
over all frames in trajectory $k$.
%
...
%
where
$N_k$ is the total number of trajectory frames
from simulation $k$;
and we have set $g$ to $1.0$,
and used $Z_i[g] = \exp(-f_i)$.
%
This is also the MBAR result\cite{
shirts2008},
generalizing the BAR result\cite{
bennett1976}($K = 2$).



The histogram-based form
can be recovered as follows.
%
Let us write $q_i(\vx)$
as $\exp[-\Ham(\vx; \lambda_i)]$,
and assume that the reduced
Hamiltonian $\Ham(\vx; \lambda_i)$
depends on $\lambda_i$ linearly:
%
\begin{equation}
...
%
where
$n_k(W) \equiv \sum^{(k)}_{\vx} \delta(\W(\vx) - W)$
is the unnormalized distribution of
$\W(\vx)$ from simulation $k$.
%
It can be estimated
from the histogram of $W$ as
the number of trajectory frames
whose $\W(\vx)$ fall in the bin
$(W - h/2, W + h/2)$
divided by the bin size $h$.
%
Since both Eqs. \eqref{eq:f_MBAR} and \eqref{eq:f_WHAM}
...
\end{equation}
where
$d_k(W) = N_k \exp(-\beta_k \, W + f_k)$.
%
Alternatively, Eq. \eqref{eq:g_WHAM}
can be derived as the optimal combination\cite{
roux1995, souaille2001, newman, frenkel} of
%
\begin{equation}
g(W)
=
...
In this study,
we shall focus on the special case of
multiple canonical ($NVT$) ensembles
with different temperatures.
%
Here,
$\Ham_0(\vx) = 0$,
$\lambda_i =\beta_i = 1/(k_B \, T)$
is the inverse temperature,
and
$\W(\vx) = \E(\vx)$ is the energy function.
%
Another common case
is a system under a quadratic restraint,
...
and thus can be added back to $f_i$ after the analysis.
...
can be generalized
to the vector form
for a few linear parameters
$\bm{\lambda}_i = \{\lambda_i^{(1)}, \lambda_i^{(2)}, \dots\}$,
and biases
$\vct W = \{W^{(1)}, W^{(2)}, \dots\}$.
%and Eq. \eqref{eq:hamlam} becomes
%\[
%\Ham(\vx; \bm{\lambda}_i)
%=
%\Ham_0(\vx)
%+
%\sum_a \lambda_i^{(a)} \, \W^{(a)}(\vx).
%\]
%
For example,
for simulations
...
the histogram-free, Eq. \eqref{eq:f_MBAR},
form is more convenient\cite{
shirts2008}.
%
Besides,
the factor $\exp(-\lambda_i \, W)$
can be replaced by
a non-Boltzmann
...
in non-canonical ensembles.



The WHAM equation \eqref{eq:f_WHAM}
is often solved iteratively as
%
\begin{equation*}
f_i^\mathrm{(new)}
=
...
\end{equation*}
%
This approach, referred to as direct WHAM below,
can be very slow
(cf. Appendix \ref{sec:convwham}),
requiring thousands of iterations\cite{
bereau2009, kim2011, zhu2012}.
%
In Sec. \ref{sec:DIIS} and \ref{sec:basis_updating},
we shall describe
a numerical technique
that accelerates the solution
of Eqs. \eqref{eq:f_MBAR} and \eqref{eq:f_WHAM}.
...
we briefly discuss
an elegant non-iterative alternative, ST-WHAM\cite{
kim2011}
and some related methods\cite{
kastner2005, *kastner2009,
shen1991, woolf1994, *crouzy1994, roux1995}.
%
By taking the logarithmic derivative of
the denominator of Eq. \eqref{eq:g_WHAM}
...
would cause a larger error in $g(W)$].
%
ST-WHAM is most convenient in one dimension,
and its result differs only slightly from WHAM\cite{kim2011}.
%
A related method, UIM\cite{
kastner2005, *kastner2009},
further approximates
the distribution $n_k(W)$ as a Gaussian.
%
%This, however, may limit the long-term accuracy.
%
The key of ST-WHAM
is to replace the weight
$d_j(W)/\sum_{k=1}^K d_k(W)$
by
$n_j(W)/\sum_{k=1}^K n_k(W)$.
%
Other methods used
a similar weighting strategy
but applied them to
$g(W)$\cite{shen1991, roux1995}
or
$\log g(W)$\cite{woolf1994, *crouzy1994, roux1995}.
...
\subsection{\label{sec:basis_updating}
Basis updating}
...
We shall test the temperature WHAM,
so $\lambda \rightarrow \beta$,
and
$W \rightarrow E$.
%
For simplicity,
...
For comparison,
we also computed the dimensionless free energies
from two approximate formulae.
%
First\cite{park2007}
%
\begin{equation}
...
(e.g., the MD program was based on
...
with parameters $\lambda_1$ and $\lambda_2$
with distributions $\rho_1(W)$ and $\rho_2(W)$
normalized as
%
\begin{equation}
\int \rho_i(W) \, dW
...
are symmetric with respect to some $\bar W$,
...
Eq. \eqref{eq:f_MBAR} or \eqref{eq:f_WHAM}
can be written as
%
\begin{align}
\Delta f
&=
-\log \int
\frac{ \rho_1(W) + \rho_2(W) }
{ \exp(\Delta \lambda \, W) + \exp \Delta f }
dW
\notag \\
&=
\Delta f
...
\end{align}
%
where $y \equiv \cosh(\Delta \lambda \, W)$,
and we have used
