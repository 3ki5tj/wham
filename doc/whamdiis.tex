...
\hl{and the closely-related multiple Bennett acceptance ratio (MBAR) method}
...
\hl{a lattice model,
a simple liquid
and an aqueous protein solution}.
\end{abstract}
...
can suffer from
slow convergence in the later stages\hl{\cite{
...
the implementation of WHAM \hl{and MBAR} using
...
the $K$ canonical ensembles\hl{\cite{chodera2007}},
...
one can show that
the $N_k$ in Eq. \eqref{eq:gE_WHAM}
should be replaced by the average, $\langle N_k \rangle$,
and the combination $\langle N_k \rangle/Z_k$
...
%
This variant is thus more convenient
as it depends only on the known weight, $\eta_k$,
instead of on the unknown $f_k$.
}
%
}
%
...
the histogram-free, or the MBAR, form\hl{\cite{
kumar1992, souaille2001, shirts2008, tan2012}}:
%
\begin{equation}
f_i
=
...
{
  \sum_{k = 1}^K N_k \, q_k(\vx) \exp \hl{(f_k)}
}.
\label{eq:f_MBAR}
\end{equation}
%
...
\hl{
In this sense,
MBAR is not only the zero-bin-width limit of WHAM\hl{\cite{shirts2008, tan2012}},
...
to be applicable to both cases.
}
%
...
%
where
\hl{$n_k(W) \equiv \sum_{\vx}^{(k)} \delta(\W(\vx) - W)$}
is understood to be
...
with $\sum_{k = 1}^K n_k(E') \, \hl{\Delta E} = 0$,
...
%
\begin{equation}
  R_i(\vct f) = 0  \qquad \mbox{for $i = 1, \ldots, K$},
  \label{eq:R_f}
\end{equation}
%
which is
...
$\vct{\hat R} = \hl{\sum_{i = 1}^M} c_i \, \vct R_i$,
...
%
\begin{equation}
  \hl{\sum_{j = 1}^M} \left( \vct R_i \cdot \vct R_j \right) \, c_j = \lambda,
  \label{eq:cj_DIIS}
\end{equation}
%
for all $i$,
...
$\vct{\hat f} = \sum_{i = 1}^M c_i \, \vct f_i$,
...
\begin{center}
  ...
  \caption{\label{fig:scheme}
    ...
    we correct $\vct f_i$ using $\vct R_i$.
    %
    ...
    (b) and (c)
    %
    Given a basis of \hl{$M$} (two here) trial vectors,
    DIIS seeks the combination
    $\vct {\hat R} = \hl{\sum_{i = 1}^M} c_i \, \vct R_i$
    that minimises the magnitude
    $\| \vct{\hat R} \|$
    under the constraint
    $\hl{\sum_{i = 1}^M} c_i = 1$ [panel (c)].
    %
    The corresponding combination
    of the trial vectors,
    $\vct {\hat f} = \hl{\sum_{i = 1}^M} c_i \, \vct f_i$,
...
\begin{center}
  ...
  \caption{
    ...
    \hl{$12.5$, $1.87\times 10^3$, $14.3$, and $260$} seconds,
    respectively.
    %
    ...
    for MBAR.
    \hl{To mimic the correlation in the trajectories,
      each data point in the bootstrap sample is
      either randomly drawn from the trajectory,
      or the same as the previous one (if any)
      with probability $\exp(-\Delta t_\mathrm{MD}/t_\mathrm{act})$,
      where $\Delta t_\mathrm{MD}$ and $t_\mathrm{act}$
      are the time step of molecular dynamics
      and the autocorrelation time of the potential energy, respectively.
    }
    %
...
denotes an average \hl{over trajectory} $i + 1$.
%
...
%
The first is\hl{\cite{park2007, zhu2012}}
%
\begin{equation}
\Delta f_i
\approx
...
which \hl{can be difficult for ST-WHAM\cite{kim2011}}.
%
...
\hl{with temperatures} $T = 1.2, 1.3, \dots, 1.7$,
\hl{and pressures} $p = 0.1, 0.15, 0.2$.
%
...
We tested the methods on a small protein,
the villin headpiece
(PDB ID: 1VII),
\hl{in aqueous solution}.
%
\hl{This} is a well-known test system\cite{duan1998}.
%
...
%
\hl{Molecular dynamics (MD)} simulations were performed
using \hl{GROMACS 5.0}\cite{
...
%
\hl{although MBAR was slower than WHAM
as it did not use histogram to aggregate data.}
%
...
\hl{which is the zero-bin-width limit of WHAM\cite{shirts2008, tan2012}.
%
...
%
Generally,
WHAM \hl{was} insensitive to the bin size,
\hl{whereas ST-WHAM} was slightly affected
by a small bin size for \hl{the} smaller samples.
...
WHAM and MBAR to produce free energy difference.
%
...
Probabilistic \hl{derivations} of Eq. \eqref{eq:f_MBAR}}



Here \hl{we give some derivations of Eq. \eqref{eq:f_MBAR}.
First,} we show that
Eq. \eqref{eq:f_MBAR}
is a generalisation of Eq. \eqref{eq:f_WHAM}
from the energy space to the configuration space.
%
We follow the probabilistic argument\hl{\cite{
bartels1997, gallicchio2005, habeck2007, habeck2012,
zhu2012, tan2012}}
for simplicity.
%
...
to the element,
\hl{
$\sum_{j = 1}^K \sum_{\vx}^{(j)} \delta(\vx - \vy) \, d\vy$,
regardless of the state $j$.
}
}
%
%
...
\hl{
We can also show Eq. \eqref{eq:f_MBAR}
without introducing the configuration-space field
[thus, $g(\vct x) = 1$ below].
%
Instead,
we now assume that each trajectory frame $\vct x$
is free to choose the state $i$ according to
the Bayes' rule\cite{leonard, duda}:
%
since the prior probability of state $i$
is $N_i/N_\mathrm{tot}$,
and the conditional probability of observing
$\vx$ in state $i$ is $w_i(\vx) \equiv q_i(\vx) / Z_i$,
%
the posterior probability of $\vx$
...
%
\begin{equation}
  \hat N_i = \sum_{\vx} p(i|\vx)
  = \sum_{\vx}
  \frac{ N_i \, w_i(\vx) }
  { \sum_{k = 1}^K N_k \, w_k(\vx) },
  \label{eq:population_match}
\end{equation}
%
where
$\sum_{\vx} \equiv \sum_{j=1}^K \sum_{\vx}^{(j)}$.
%
If we demand $\hat N_i$ to be equal to the $N_i$,
Eq. \eqref{eq:f_MBAR} is recovered.
%
Alternatively, by following the argument used
in the expectation-maximisation algorithm\cite{
dempster1977, neal1998,
duda, press3rd, habeck2012},
we can view Eq. \eqref{eq:population_match}
as the result of maximising
the probability of observing the trajectory $\{ \vx \}$,
%
\[
  p\left( \{\vx \} | N_1, \dots, N_K \right)
  \propto
  \prod_{\vx}
  \sum_{ k = 1 }^K
    ( N_k / N_\mathrm{tot} ) \, w_k(\vx),
\]
%
with respect to variations of $N_i$
under the constraint $\sum_{i = 1}^K N_i = N_\mathrm{tot}$.
%
}
...
as a Gaussian\hl{\cite{bereau2009}}.
%
...
can be set to any values that help \hl{calculation},
...
and
\hl{$D(E; \vct f^*) = \exp\left( \frac{1}{2} \Delta \beta E \right) J(E)$},
with
\begin{align*}
  \hl{J(E)}
  &\hl{\equiv}
...
\end{align*}
where
\hl{ $\theta = 1 + \frac 1 2 \exp\left[-u \Delta \beta^2 / (4 \, a) \right]$ }.
%
...
%
\hl{
Besides, sampling error,
which is ignored in the above calculation,
may also slow down the convergence.
}
