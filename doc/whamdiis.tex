...
to a closely related method,
the multiple Bennett acceptance ratio (MBAR) method.
}
%
...
In this sense,
MBAR is
not only a special form or the zero-bin-width limit of WHAM,
...
as we shall see,
permits our acceleration technique to be
...
\add{Derivations} of Eq. \eqref{eq:f_MBAR}}
...
We can \textcolor{red}{alternatively} show Eq. \eqref{eq:gy}
...
\add{
It is also possible to directly show Eq. \eqref{eq:f_MBAR}
from the composite ensemble viewpoint
without using the phase-space field, $g(\vct x)$.
%
Instead, we allow each trajectory frame $\vct x$
to choose the state $i$ according to
the weight in the composite ensemble.
%
Then, the probability of $\vx$
being in state $i$ is given by
\[
  p(i|\vx)
