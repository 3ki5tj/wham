...
%
We shall show that
the convergence is slowed down when the methods
are applied to a wide temperature range.
...
$\beta_i$ ($i = 1, \dots, K$),
%
each of which is normalized as
%
%\begin{equation}
$
\int \rho_i(E) \, dE = 1.
...
%
For simplicity,
we assume equal number of visits
to all temperatures.
%
Then, the WHAM equation,
Eq. \eqref{eq:f_WHAM},
can be written
...
$\lambda_1$,
determines the slowest mode of error reduction,
%
and a smaller $\lambda_1$
...
the two-temperature value
given by Eq. \eqref{eq:lambda1_twoT}
for a large range.
%
Thus,
Eq. \eqref{eq:lambda1_cont}
only gives a lower bound,
