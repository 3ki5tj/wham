...
\end{align}
%
which involves no $f_k$,
for $d_k(E)$ has been replaced by $n_k(E)$
in the second term
...
may vanish for some bin.
%
In this case, we let integrand inherit
the last valid value of smaller $E'$
(setting it to zero would cause a larger error).
%
...
An instructive special case is $M = 2$, and $K = 1$,
which reduces the method to the secant method\cite{press}.
...
%
All methods were insensitive to the bin size
within the reasonable range.
%
ST-WHAM and UIM produced similar and reasonable results,
with the former being slightly better.
%
The approximate Eq. \eqref{eq:df_eav}
was less accurate,
although its improvement
Eq. \eqref{eq:df_eavb},
was comparable to ST-WHAM and UIM.
%
...
\begin{figure}[h]
  ...
  \caption{
    ...
    for the villin headpiece
    %
    The bin size of the energy histograms is $h = 1.0$.
    %
...
from the energy space to the phase space.
%
...
an unknown underlying phase-space field, $g(\vy)$,
